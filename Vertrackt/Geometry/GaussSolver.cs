...
namespace Vertrackt.Geometry
{
    public static class GaussSolver
    {
        ...
        {
            double[] result;
            double x3 = -m[1, 1] * m[2, 0] * m[0, 3] + m[0, 0] * m[1, 1] * m[2, 3] - m[0, 0] * m[2, 1] * m[1, 3] +
            ...
            double x1 = -m[0, 1] * m[2, 2] * m[1, 3] + m[0, 1] * m[2, 3] * m[1, 2] - m[0, 2] * m[1, 1] * m[2, 3] +
                        m[0, 2] * m[2, 1] * m[1, 3] - m[0, 3] * m[2, 1] * m[1, 2] + m[0, 3] * m[1, 1] * m[2, 2];

            double denom = -m[0, 0] * m[2, 1] * m[1, 2] + m[0, 0] * m[1, 1] * m[2, 2] + m[2, 0] * m[0, 1] * m[1, 2]
        ...
        {
            double[] result;
            double x2 = m[0, 0] * m[1, 2] - m[1, 0] * m[0, 2];
            double x1 = -(m[0, 1] * m[1, 2] - m[0, 2] * m[1, 1]);
        ...
        private static double[] DefaultAlgorithm(double[,] m, int order)
        {
            ...
            for (int i = 0; i < order; i++)
            {
                ...
                for (int j = i + 1; j < order; j++)
                {
                    temp = m[j, i] / m[i, i];
                    m[j, i] = 0;
                    for (int k = i + 1; k < orderPlusOne; k++)
                    {
